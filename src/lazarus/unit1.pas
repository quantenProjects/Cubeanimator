...
uses
  ...
  StdCtrls, types, LCLType, Spin, Clipbrd, lclintf;
...
var begining,ending,s:string;
  tfbegin,tfend,tfOut: TextFile;
begin
  //begining:='hallo     geht das ueber mehere zeilen';
  AssignFile(tfbegin,'website_begining.html');
  AssignFile(tfend,'website_ending.html');
  AssignFile(tfOut,'website.html');
  reset(tfbegin);
  Reset(tfend);
  Rewrite(tfOut);
  while not eof(tfbegin) do
    begin
      readln(tfbegin, s);
      writeln(tfOut,s);
    end;
  WriteLn(tfOut,'var animation_data = ',export_array_js,';');
  while not eof(tfend) do
    begin
      readln(tfend, s);
      writeln(tfOut,s);
    end;
